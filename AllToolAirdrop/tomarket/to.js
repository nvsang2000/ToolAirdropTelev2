...
class Tomarket {
    ...
    async checkTicket(initData) { 
        const url = 'https://api-web.tomarket.ai/tomarket-game/v1/user/tickets'; 
        const data = JSON.stringify({ language_code: 'en' }); 
        try { 
            const res = await this.http(url, this.headers, data); 
            if (res.status === 200 && res.data.status === 0) { 
                const tickets = res.data.data.ticket_spin_1; 
                this.log(`Bạn có ${tickets} vé quay...`); 
                if (tickets > 0) { 
                    for (let index = 0; index < tickets; index++) {
                        await this.spin(initData); 
                        await this.countdown(3);
                    }
                } 
            } 
            ...
            this.log(colors.red(`Lỗi khi lấy số vé quay: ${error.message}`)); 
            return null; 
        } 
    }
    async spin(init_data){ 
        const url = 'https://api-web.tomarket.ai/tomarket-game/v1/spin/raffle'; 
        const data = JSON.stringify({ category: 'ticket_spin_1'}); 
        try { 
            const res = await this.http(url, this.headers, data); 
            if (res.status === 200 && res.data.status === 0) { 
                const result = res.data.data.results; 
                result.forEach(item => { 
                    this.log(`Bạn nhận được ${item.amount} ${item.type}...`); 
                }); 
                 
            } 
            return null; 
        } catch (error) { 
            this.log(colors.red(`Lỗi khi quay: ${error.message}`)); 
            return null; 
        } 
    }
    async main() {
