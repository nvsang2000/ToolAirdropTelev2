    ...
    async nangcap(telegramauth, balance) {
        ...
        try {
            ...
            if (incubationInfo.status === "processing") {
                // console.log(currentTime > upgradeCompletionTime);
                // return;
                if (currentTime > upgradeCompletionTime) {
                    const confirmResponse = await axios.post("https://api.birds.dog/minigame/incubate/upgrade", {}, { headers });
                    this.log("Đang yêu cầu nâng cấp trúng");
    ...
    async main() {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                const telegramauth = data[i].trim();
