...
class MatChainNFTClaimer {
    ...
    async claimNFT(token, walletAddress) {
        try {
            const claimUrl = "https://tgapp-api.matchain.io/api/tgapp/v1/megadrop/megadrop/nft/psg/claim";
    ...
    async processAccount(loginData, walletAddress, privateKey) {
        try {
            ...
            console.log(`Processing account with wallet: ${cleanWallet}`);
    ...
    async main() {
        ...
        const privateKeys = this.readAndCleanFile('dancay.txt');
