    ...
    for (const category of marketplaceData.data.categories) {
      if (category.cards) {
        const filteredCards = category.cards.filter(
          (card) =>
            !card.hasStartDependency && // Chỉ lấy thẻ không có dependency
            !card.isCompleted && // Chưa hoàn thành
            card.cost <= balance && // Đảm bảo đủ balance để mua
            card.currentLevel < card.maxLevel && // Kiểm tra chưa đạt level tối đa
            card.cooldownTime === 0 // Đảm bảo không có cooldown
        );
        eligibleCards = [...eligibleCards, ...filteredCards];
      }
    }

    // Sắp xếp thẻ theo profit giảm dần
    eligibleCards.sort((a, b) => b.profit - a.profit);

    // Mua từng thẻ theo thứ tự
    for (const card of eligibleCards) {
      if (balance >= card.cost) {
        ...
        if (
          ...
          balance = purchaseResult.data.balance; // Cập nhật balance sau mỗi lần mua
          this.log(
