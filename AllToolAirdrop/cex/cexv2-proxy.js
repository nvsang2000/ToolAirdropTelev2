...
class CexAPI {
    constructor() {
        this.headers = {
            ...
            "Origin": "https://cexp10.cex.io",
            "Referer": "https://cexp10.cex.io/",
    ...
    async convertCrypto(authData, Dancay, userData) {
        ...
        const divisor = Math.pow(10, userData.precision_BTC);
        const fromAmount = (userData.balance_BTC / divisor).toFixed(userData.precision_BTC);
            ...
            for (let i = 0; i < data.length; i++) {
                try {
                    ...
                    if (result.status === "ok") {
                        ...
                        const divisor = Math.pow(10, userData.precision_BTC);
                        this.log(`Balance BTC: ${userData.balance_BTC / divisor}`, 'success');
                        ...
                        if (hoiswap) {
                            if (userData.balance_BTC > 0 && swapPercentage > 0) {
                                try {
                                    const btcToConvert = ((userData.balance_BTC / divisor) * (swapPercentage / 100)).toFixed(userData.precision_BTC);
