...
class AgentAPI {
    ...
    async waitWithCountdown(seconds) {
        for (let i = seconds; i >= 0; i--) {
            ...
            process.stdout.write(`[${new Date().toLocaleTimeString()}] [301] Chờ ${i} giây để tiếp tục...`);
    ...
    async getMe(authorization) {
        try {
            return await this.makeRequest('POST', `${this.baseURL}/getMe`, {"referrer_id": 5409014227}, authorization);
        } catch (error) {
    ...
    async main() {
        ...
        const coverData = fs.readFileSync(dataFile, 'utf8')
            .replace(/\r/g, '')
            .split('\n')
            .filter(Boolean);
        
        const data = coverData.map((item, index) => item.trim())
