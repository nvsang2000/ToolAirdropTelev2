...
class YesCoinBot {
    constructor(accountIndex, account) {
    ...
    async getOrRefreshToken(encodedData) {
        ...
        this.token = await this.login(encodedData);
    ...
    async collectCoin(token, amount) {
        ...
        try {
            const response = await this.makeRequest('post', url, amount, token);
            if (response.code === 0) {
                return response;
            }
            return null;
    ...
    async getAccountInfo(token) {
        try {
            ...
            const response = await this.makeRequest('get', url, null, token);
            if (response.code === 0) {
                return response;
            }
            return null;
    ...
    async collectFromSpecialBox(token, boxType, coinCount) {
        ...
        try {
            const response = await this.makeRequest('post', url, data, token);
            if (response.code === 0) {
    ...
    async finishTask(token, taskId) {
        ...
        try {
            const response = await this.makeRequest('post', url, taskId, token);
            if (response.code === 0) {
    ...
    async upgradeLevel(token, currentLevel, targetLevel, upgradeType) {
        ...
        while (currentLevel < targetLevel) {
            try {
                const response = await this.makeRequest('post', url, upgradeType, token);
                if (response.code === 0) {
                    currentLevel++;
        ...
        try {
            ...
            try {
                this.token = await this.performTaskWithTimeout(
                    () => this.getOrRefreshToken(this.account),
                    'Getting token',
                    20000
                );
            } catch (error) {
    ...
    async checkAndClaimTaskBonus(token) {
    ...
    try {
        ...
        if (bonusInfo.commonTaskBonusStatus === 1) {
            const claimResponse = await this.makeRequest('post', claimUrl, 2, token);
            if (claimResponse.code === 0) {
        ...
        if (bonusInfo.dailyTaskBonusStatus === 1) {
            const claimResponse = await this.makeRequest('post', claimUrl, 1, token);
            if (claimResponse.code === 0) {
    ...
    async performDailyMissions(token) {
        try {
            ...
            const dailyMissionsResponse = await this.makeRequest('get', dailyMissionsUrl, null, token);
            ...
            if (dailyMissionsResponse.code === 0) {
                for (const mission of dailyMissionsResponse.data) {
                    if (mission.missionStatus === 0) {
                        ...
                        await this.makeRequest('post', clickUrl, mission.missionId, token);
    
                        const checkUrl = 'https://api-backend.yescoin.gold/mission/checkDailyMission';
                        const checkResponse = await this.makeRequest('post', checkUrl, mission.missionId, token);
    
                        if (checkResponse.code === 0 && checkResponse.data === true) {
                            const claimUrl = 'https://api-backend.yescoin.gold/mission/claimReward';
                            const claimResponse = await this.makeRequest('post', claimUrl, mission.missionId, token);
    ...
    async performDailySignIn(token) {
        try {
            ...
            const signInListResponse = await this.makeRequest('get', signInListUrl, null, token);
            ...
            if (signInListResponse.code === 0) {
                ...
                if (availableSignIn) {
                    ...
                    const signInResponse = await this.makeRequest('post', signInUrl, signInData, token, headers);
        ...
        try {
            ...
            if (squadInfo && squadInfo.data.isJoinSquad) {
                ...
                const joinResult = await this.performTaskWithTimeout(
                    () => this.joinSquad(this.token, "t.me/dancayairdrop"),
                    'Joining squad',
                    20000
                );
                if (joinResult) {
            ...
            await this.performTaskWithTimeout(
                () => this.performDailySignIn(this.token),
            ...
            const gameInfo = await this.performTaskWithTimeout(
                () => this.getAccountBuildInfo(this.token),
                'Getting game info',
                15000
            );
            if (gameInfo === null) {
            ...
            await this.performTaskWithTimeout(
                () => this.handleSwipeBot(this.token),
                'Handling SwipeBot',
                30000
            );

            await this.performTaskWithTimeout(
                () => this.performDailyMissions(this.token),
            ...
            await this.performTaskWithTimeout(
                () => this.checkAndClaimTaskBonus(this.token),
            ...
            if (this.config.upgradeMultiEnable && gameInfo) {
                await this.performTaskWithTimeout(
                    () => this.upgradeLevel(this.token, gameInfo.data.singleCoinValue, this.config.maxLevel, '1'),
                    'Upgrading Multi',
                    60000
                );
            }
            ...
            if (this.config.upgradeFillEnable && gameInfo) {
                await this.performTaskWithTimeout(
                    () => this.upgradeLevel(this.token, gameInfo.data.coinPoolRecoverySpeed, this.config.maxLevel, '2'),
                    'Upgrading Fill',
                    60000
                );
            }
            ...
            if (collectInfo === null) {
                ...
                if (coinPoolLeftCount > 0) {
                    ...
                    const collectResult = await this.performTaskWithTimeout(
                        () => this.collectCoin(this.token, amount),
                        'Collecting coins',
                        30000
                    );
            ...
            if (updatedGameInfo && updatedGameInfo.data.coinPoolLeftRecoveryCount > 0) {
                const recoverResult = await this.performTaskWithTimeout(
                    () => this.recoverCoinPool(this.token),
            ...
            const freeChestCollectedAmount = await this.performTaskWithTimeout(
                () => this.attemptCollectSpecialBox(this.token, 1, 200),
...
if (isMainThread) {
    ...
    const bot = new YesCoinBot(workerData.accountIndex, workerData.account);
